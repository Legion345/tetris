...
void save(tetris_game *game, WINDOW *w) {
  ...
  printf("Resume by passing the filename as an argumet to this program.\n");
  exit(EXIT_SUCCESS);
}
...
int main(int argc, char **argv) {
  ...
  if (SDL_Init(SDL_INIT_AUDIO) < 0) {
    fprintf(stderr, "unable to intialize SDL\n");
    exit(EXIT_FAILURE);
  }
  if (Mix_Init(MIX_INIT_MP3) != MIX_INIT_MP3) {
    fprintf(stderr, "unable to intialize SDL_mixer\n");
    exit(EXIT_FAILURE);
  }
  if (Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 1024) != 0) {
    fprintf(stderr, "unable to intialize audio\n");
    exit(EXIT_FAILURE);
  }
  ...
  keypad(stdscr, TRUE); // initialize curses
  timeout(0);           // initialize curses
  curs_set(0);          // initialize curses
  init_colors();        // initialize curses
